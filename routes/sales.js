...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (Array.isArray(sale.batchesUsed)) {
        for (const bu of sale.batchesUsed) {
          ...
          if (snap.exists) {
            ...
            missing.push(bu);                   // recreate later
          }
        }
      }
      ...
      /* recreate fully missing batches */
      ...
      await Promise.all([
        ...
        cacheDel(`products_${sale.accountId}`),
        cacheDel(`product_${productId}`)
      ]);

      const monthTotal = await computeMonthTotal(
        sale.accountId, sale.saleDate.substring(0,7)
      );
      const { summary } = await computeDailySummary(
